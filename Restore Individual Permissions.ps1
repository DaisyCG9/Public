...
$AccessRule = New-Object System.Security.AccessControl.FileSystemAccessRule("CARMEUSE\$($secadd.SamAccountName)","$permtype",'ContainerInherit, ObjectInherit','none','Allow')
...
$permtype = Read-Host -Prompt "Enter the type of permissions to grant (ReadandExecute, Modify, Full)"
$secadd = Get-ADGroup -Filter "Name -eq '$user'" -Properties SamAccountName
$AccessRule = New-Object System.Security.AccessControl.FileSystemAccessRule("CARMEUSE\$($secadd.SamAccountName)","$permtype",'None','none','Allow')
$acl.SetAccessRule($AccessRule)
$acl | Set-Acl d:\
Pause