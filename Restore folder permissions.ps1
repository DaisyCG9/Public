...
Foreach ($name in $foldernames) {
    ...
    $AccessRule = New-Object System.Security.AccessControl.FileSystemAccessRule("CARMEUSE\$($user.SamAccountName)",'ContainerInherit, ObjectInherit','1',"Allow")
...
Foreach ($name in $foldernames) {
    ...
    $accessrule = New-Object system.security.AccessControl.FileSystemAccessRule("carmeuse\AWLONGWORTH","Read",,,"Allow")
