...
Enable-csuser -Identity carmeuse\JSMITH -Sipaddress SIP:JSMITH@carmeuse.net -RegistrarPool eur-skype-pool.carmeuse.net
Grant-CsConferencingPolicy -identity carmeuse\JSMITH -PolicyName "Enterprise CAL - VoIP Only"
Grant-CsExternalAccessPolicy -Identity carmeuse\JSMITH -PolicyName "Enabled external access"
Grant-CsMobilityPolicy -Identity carmeuse\JSMITH -PolicyName "Allow connection from smart devices"

Get-csuser -Identity carmeuse\bdesmarets

#add a delegate in skype for business
Set-CsUser -Delegates @{add='karen.dicaprio@carmeuse.com'} -User "thomassibo@carmeuse.com"
