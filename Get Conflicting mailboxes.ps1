...
Foreach ($email in $emails){
    ...
    If ($email.PrimarySmtpAddress.Domain -like "carmeusena*"){
        Write-Output $email.PrimarySmtpAddress
        #$output1 = $email.PrimarySmtpAddress
        #Add-Content -Path C:\Users\admqmfavo\Desktop\emailswithout.txt -Value $output1
        If ($email.EmailAddresses.SmtpAddress -like "*carmeuse.com"){
        Write-Output "$($email.EmailAddresses.SmtpAddress) has a carmeuse.com email alias"
...
#add carmeuse.com alias to emails
...
Foreach ($tochange in $emailchange){
    ...
    $address = $mailbox.alias + "@carmeuse.com"
    $mailbox | Set-mailbox -EmailAddresses @{add="$address"}
}

#add carmeuse.com alias to DLs
...
Foreach ($tochange in $DLchange){
    ...
    $DGaddress = $DGroup.alias + "@carmeuse.com"
...
Foreach ($test in $testing){
    If ($test -like $testing2[$i]){
        ...
        Write-Output "$test needs carmeuse.com"
        $i --
    }
    $i ++
}